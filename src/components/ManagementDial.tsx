...
interface ManagementDialProps {
  value: number;
  onChange: (value: number) => void;
}

export const ManagementDial: React.FC<ManagementDialProps> = ({ value, onChange }) => {
  const managementLevels = [
    { level: 0, label: 'Pasyvus valdymas', description: 'Retai tikrinti', icon: Clock, color: 'text-green-600', bg: 'bg-green-100' },
    { level: 1, label: 'Vidutinis valdymas', description: 'Tikrinti kas mėnesį', icon: Settings, color: 'text-blue-600', bg: 'bg-blue-100' },
    { level: 2, label: 'Aktyvus valdymas', description: 'Nuolatinis stebėjimas', icon: Wrench, color: 'text-orange-600', bg: 'bg-orange-100' }
  ];
